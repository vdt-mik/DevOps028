  ...
  stages {
    ...
    stage('create_ec2-instance') {
      ...
      steps {
        ...
        ZONE = sh (
            script: 'aws ec2 describe-availability-zones --query 'AvailabilityZones[0].ZoneName' --profile $MY_PROFILE --output text',
            returnStdout: true
        ).trim()
        sh 'echo $ZONE'
        ...
        lb_name = sh (
            script: 'aws elb describe-load-balancers --load-balancer-names $ELB_NAME --query 'LoadBalancerDescriptions[].DNSName' --profile $MY_PROFILE --output text',
            returnStdout: true
        ).trim()
