  ...
  stages {
    ...
    stage('create_ec2-instance') {
      ...
      steps {
        ...
        sh 'ZONE=$(aws ec2 describe-availability-zones --query 'AvailabilityZones[0].ZoneName' --profile $EUCA_PROFILE --region $MY_REGION --output text)'
        sh 'echo $ZONE'
        ...
        sh 'lb_name=$(aws elb describe-load-balancers --load-balancer-names $ELB_NAME --query 'LoadBalancerDescriptions[].DNSName' --profile $MY_PROFILE --output text)'
